...
import com.example.sipsupporterapp.databinding.FragmentCostBinding;
...
import com.example.sipsupporterapp.view.dialog.AddEditCostDialogFragment;
import com.example.sipsupporterapp.view.dialog.DeleteCostQuestionDialogFragment;
import com.example.sipsupporterapp.view.dialog.ErrorDialogFragment;
import com.example.sipsupporterapp.view.dialog.SuccessDeleteCostDialogFragment;
    ...
    private void handleEvents() {
        binding.fabAddCost.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                AddEditCostDialogFragment fragment = AddEditCostDialogFragment.newInstance(0, "", 0, 0, 0, 0, "");
                fragment.show(getParentFragmentManager(), AddEditCostDialogFragment.TAG);
            }
        });
    ...
    private void setupObserver() {
        ...
        viewModel.getEditClickedSingleLiveEvent().observe(getViewLifecycleOwner(), new Observer<PaymentInfo>() {
            ...
            public void onChanged(PaymentInfo paymentInfo) {
                AddEditCostDialogFragment fragment = AddEditCostDialogFragment.newInstance(paymentInfo.getPaymentID(), paymentInfo.getDescription(), paymentInfo.getDatePayment(), paymentInfo.getPrice(), paymentInfo.getBankAccountID(), paymentInfo.getPaymentSubjectID(), paymentInfo.getPaymentSubject());
                fragment.show(getParentFragmentManager(), AddEditCostDialogFragment.TAG);
            }
        });
        ...
        viewModel.getDeleteClicked().observe(getViewLifecycleOwner(), new Observer<PaymentInfo>() {
            ...
            public void onChanged(PaymentInfo paymentInfo) {
                ...
                DeleteCostQuestionDialogFragment fragment = DeleteCostQuestionDialogFragment.newInstance(getString(R.string.question_delete_cost_message));
                fragment.show(getParentFragmentManager(), DeleteCostQuestionDialogFragment.TAG);
            }
        });
        ...
        viewModel.getPaymentDeleteResultSingleLiveEvent().observe(getViewLifecycleOwner(), new Observer<PaymentResult>() {
            @Override
            public void onChanged(PaymentResult paymentResult) {
                SuccessDeleteCostDialogFragment fragment = SuccessDeleteCostDialogFragment.newInstance(getString(R.string.success_delete_cost_message));
                fragment.show(getParentFragmentManager(), SuccessDeleteCostDialogFragment.TAG);
                fetchCostsByBankAccountID();
            }
        });
        ...
        viewModel.getSeeDocumentsClicked().observe(getViewLifecycleOwner(), new Observer<PaymentInfo>() {
            @Override
            public void onChanged(PaymentInfo paymentInfo) {
                Intent starter = ImageGalleyContainerActivity.start(getContext(), 0, 0, 0, paymentInfo.getPaymentID());
            ...
            public void onChanged(Integer bankAccountID) {
                for (int i = 0; i < bankAccountInfoArray.length; i++) {
                    if (bankAccountInfoArray[i].getBankAccountID() == bankAccountID) {
                        CostFragment.this.bankAccountID = bankAccountID;
                    }
                }
